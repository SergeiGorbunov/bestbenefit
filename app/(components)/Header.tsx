...
const Header = () => {
  ...
  return (
    <>
      <header className="fixed top-0 w-full z-50 backdrop-blur-md bg-black/90 border-b border-orange-900 py-4">
        <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <nav className="hidden md:flex space-x-8">
            <Link
              href="/"
              className="hover:text-orange-400 transition-all duration-300 hover:scale-105"
            >
              Главная
            </Link>
            <Link
          ...
          <div className="flex items-center gap-4">
            ...
            >
              <span
                className={`absolute w-6 h-0.5 bg-orange-500 transition-all duration-300 ${
      ...
      {/* Мобильное меню */}
      <div
        className={`fixed inset-0 z-40 bg-black/90 backdrop-blur-xl transition-all duration-700 ease-in-out ${
        ...
        <nav className="h-full flex flex-col items-center justify-center space-y-8">
          {[
            ...
            <a
              key={index}
              href={item.href}
              className={`text-3xl font-bold text-white transform transition-all duration-700 ${
                isMenuOpen
